...
echo -e "\tGit / Cdn ................ : Sources from kernel.org"
echo -e "\t\033[35mckc-\033[3mVersion-Label\033[m ........ : Local custom kernel packages"
echo ""
echo -e "Database update :\033[34m" $(stat $RemoteVersion|grep ^Modif|cut -d: -f2-|cut -d. -f1) "\033[m(krn Update to refresh database)"
echo ""

cp $RemoteVersion $TempDir/VersionBDD
for Param in $*
do
    grep "$Param" $TempDir/VersionBDD     > $TempDir/VersionBDD.tmp
    mv -f         $TempDir/VersionBDD.tmp   $TempDir/VersionBDD
done

cat $TempDir/VersionBDD|\
    while read Record
    do
...
rm $TempDir/VersionBDD

NbFound=$(ls -1 $TempDir|wc -l)
if [ $NbFound -eq 0 ]
then
