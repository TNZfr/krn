...
//------------------------------------------------------------------------------
int CSV_ParseOneRecord (char *Record, char  FieldDelimiter, _CSVPARSE *ParseData)
{
  int       NbField;
  int       Index;
  ...
  memset (ParseData, 0, sizeof(_CSVPARSE));
  CSV_CleanRecord (Record);
  ...
  if (NbField > ParseData->MaxField)
      ...
      if (ParseData->MaxField == 0)
	{
	  ...
	  if (!ParseData->Field)
	    {
	      printf ("CSV_ParseOneRecord: Memory allocation error for %d field(s)\n",NbField);
	      return 0;
	    }
	  ParseData->MaxField = NbField;
	}
      else
	{
	  ...
	  if (!NewPtr)
	    {
	      printf ("CSV_ParseRecord: Memory re-allocation error (from %d to %d field(s))\n",
		      ParseData->MaxField, NbField);
	      return 0;
	    }
	  ParseData->Field    = NewPtr;
